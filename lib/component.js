...
function Component(options) {
    ...
    this.json = {};
    try {
        var p = path.resolve(options.dir, options.json);
        this.json = readJson(p);
        logtrace("[bower   ] reading " + p);
        // logtrace(this.json);
    } catch (err) {
        // ok
    }
    try {
        var p = path.resolve(options.dir, options.composerJson);
        var jsonComposer = readJson(p);
        this.json = assign({}, jsonComposer || {}, this.json);
        logtrace("[composer] reading " + p);
        // logtrace(jsonComposer);
    } catch (err) {
        // ok
    }
    ...
    if (Object.keys(this.json).length == 0) {
        ...
        if (["ext", "php"].indexOf(prefix) == -1) {
            logtrace("----------");
            logtrace("[!!!!!!!!] no json for " + this.packetID);
            logtrace("----------");
        }
    }
    ...
    if (mainFiles.length > 0) {
        logtrace("           adding assets via `main` from " + this.packetID);
        dumpComplex(mainFiles, dumpPrefix, dumpCap, "assets by " + this.packetID);
    }
    ...
    if (options.includeFiles && this.json.files) {
        ...
        if (filesFiles.length > 0) {
            logtrace("           adding assets via `files` from " + this.packetID);
            dumpComplex(filesFiles, dumpPrefix, dumpCap, "assets by " + this.packetID);
        }
    }

    var depLen = (this.json.dependencies) ? Object.keys(this.json.dependencies).length : 0;
    if (depLen > 0) {
        logtrace("[bower   ] adding packets via `dependencies` for " + this.packetID);
        dumpComplex(this.json.dependencies, dumpPrefix, dumpCap, "packets by " + this.packetID);
    }
    ...
    if (options.isRoot) {
        ...
        if (depLen > 0) {
            logtrace("[bower   ] adding packets via `devDependencies` for " + this.packetID);
            dumpComplex(this.json.devDependencies, dumpPrefix, dumpCap, "packets by " + this.packetID);
        }
    ...
    if (reqLen > 0) {
        logtrace("[composer] adding packets via `require` for " + this.packetID);
        dumpComplex(this.json.require, dumpPrefix, dumpCap, "packets by " + this.packetID);
...
Component.prototype = {
    ...
    getDependencies: function (options) {
        ...
        logtrace('[req.for ] ' + this.filepath + " " + this.packetID);
        dumpComplex(dependencyNames, dumpPrefix, 100 + dumpCap, "dependency names");
...
function dumpComplex(complex, prefix, cap, label) {
            ...
            if (len < cap) {
                for (var key in complex) {
                    if (complex.hasOwnProperty(key)) {
                        dumpComplex(complex[key], prefix + key + ": ", cap - 1, label + "." + key);
                    }
                }
            }
            else {
                logtrace(prefix + len + " " + label + " (Object)");
            }
            break;
        case "array":
            ...
            if (len < cap) {
                complex.forEach(
                    function (value, key) {
                        dumpComplex(value, prefix + key + ": ", cap - 1, label + "." + key);
                    }
                );
            } else {
                logtrace(prefix + len + " " + label + " (Array)");
            }
            break;
        default:
            logtrace(prefix + complex);
            break;
    }
}
